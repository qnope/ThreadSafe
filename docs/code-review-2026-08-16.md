...
### T1. `#include <complex>` in `vocabulary.h:4` — the single biggest lever, and the specialization is redundant
**Measured.** Marginal cost over the library's 22-header std closure: **+0.282 s**; next largest line item is `<ranges>` at **0.033 s** — a 9× gap. It drags `<sstream>`/`<ostream>` at depth 2, 356 headers total. Per-header TU: `sendable.h` 0.774 → 0.494, `synchronizable.h` 0.743 → 0.463, `lifetime_aware.h` 0.718 → 0.459. **Suite 7.245 → 5.580 s (−23%).**

**Redundant.** A 25-assertion differential (complex of double/float/long double, of a plain struct, of a user-copy struct, of a borrowed type, const/array/vector/reference/pointer forms) gives *identical* answers with line 4 and lines 44-47 deleted — the default recursion computes exactly `is_sendable<T>` / `is_lifetime_aware<T>` in both the libstdc++-specialized and primary-template cases, so deletion cannot create the cross-TU divergence the IFNDR rule guards against.

**Fix.** Delete `vocabulary.h:4` and lines 44-47. **And rewrite `tests/test_soundness_regressions.cpp:179`,** whose message reads `"__complex__ T is neither scalar nor class; it needs its own rule"` — that is factually false on GCC 16.1.0, which reports `__complex__ double` as `scalar=1 arith=1 fundamental=1 class=0`. The assertion still passes without the specialization; keep it as the regression pin with a corrected message. Record in CLAUDE.md that this rests on a GCC classification of a language extension, whose failure mode if it changes is the loud `"supports only scalar, class and union types"` exception at `sendable.h:63`, not a wrong answer.
...
|---|---|---|---|
| Full suite (10 TUs) | 7.245 s | 5.563 s | −23% |
...
Runner-up worth landing alongside, because it costs nothing: deleting `#include <complex>` from `vocabulary.h` — a verified −23% on the whole suite, with a 25-assertion differential proving the specialization it enabled was value-identical to the default recursion. Just correct `test_soundness_regressions.cpp:179` in the same commit, since its stated rationale for that rule is measurably false.